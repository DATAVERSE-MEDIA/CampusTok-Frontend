...
export interface CurrentUserResponse {
  user: {
    ...
    id: string
    is_verified: boolean
    role: 'institution' | 'student' | 'general'
...
export const useLogin = () => {
  ...
  return useMutation({
    ...
    onSuccess: (data, variables) => {
      ...
      // Save token
      if (data.token || data.user?.campustalk_access_token) {
        const token = data.token || data.user?.campustalk_access_token
...
export const useCurrentUser = () => {
  ...
  return useQuery<CurrentUserResponse>({
    ...
    // staleTime: 5 * 60 * 1000, // 5 minutes
    // retry: 1,
    // // Initial data based on Zustand store
    // initialData: user ? {
    //   user: {
    ...
    //     role: (user.role || user.userType || 'general') as 'institution' | 'student' | 'general',
    ...
    if (query.data && query.isSuccess) {
      ...
      if (query.data.user.campustalk_access_token) {
        apiClient.defaults.headers.common['Authorization'] = 
          `Bearer ${query.data.user.campustalk_access_token}`
      }
    }
