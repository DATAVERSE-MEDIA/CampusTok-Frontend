...
export default function GeneralDashboard() {
  ...
  useEffect(() => {
    if (prevSchoolRef.current !== selectedSchool?.id) {
      console.log(
        "[GeneralDashboard] School changed from",
  ...
  const fetchPosts = async () => {
    ...
    try {
      ...
      if (selectedSchool?.id) {
        console.log(
          "[GeneralDashboard] Fetching posts for institution:",
          selectedSchool.id,
        );
        ...
        console.log("[GeneralDashboard] Fetching general feed");
        response = await apiClient.get("/posts", {
          params: { skip, limit: 10 },
        });
      }

      console.log("[GeneralDashboard] API Response:", response.data);
      const newPosts = Array.isArray(response.data)
        ? response.data
        : response.data.data || [];
      console.log("[GeneralDashboard] Posts count:", newPosts.length);
        ...
        setPosts([
          {
            ...
            author: {
              ...
              role: "student",
              school: selectedSchool?.name || "University of Lagos",
      ...
      console.error("[GeneralDashboard] Error fetching posts:", err);
      // Use dummy data for testing matching Figma
      setPosts([
        {
          id: 1,
          content:
            "When people talk about university, they often focus only on GPAs, exams, and deadlines. But real growth happens outside the classroom. It's in the late-night study sessions where friendships form, in the student clubs where leadership skills emerge, and in the moments when you step out of your comfort zone to try something new.",
          author: {
            full_name: "Farinloye Joseph",
            profile_picture: null,
            role: "student",
              ...
              <div className="text-center px-4">
                ...
                <div className="flex items-center justify-center gap-2 mb-4">
                  {[...Array(6)].map((_, i) => (
                    <div
                      key={i}
                      className="w-8 h-8 bg-amber-700 rounded-sm shadow-md"
                    ></div>
                  ))}
                </div>
                <div className="flex items-center justify-center gap-2 mb-2">
                  {[...Array(8)].map((_, i) => (
                    <div
                      key={i}
                      className="w-8 h-8 bg-amber-800 rounded-sm shadow-md"
                    ></div>
                  ))}
                </div>
                {/* Screen/Board */}
